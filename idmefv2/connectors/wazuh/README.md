...
When an alert is generated by Wazuh, a new JSON record will be appended to the `alerts.json` file. This record will be analyzed by the connector and converted to a IDMEFv2 alert that will be sent to a IDMEFv2 server (either the testserver or the Concerto SIEM).
