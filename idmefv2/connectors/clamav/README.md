...
The Clamav connector includes unit tests in [clamavconverter_test.py](./clamavconverter_test.py). Run them from the project root with:

``` sh
python3 -m pytest idmefv2/connectors/clamav/clamavconverter_test.py
```


## Running unit tests

Unit tests for the Clamav connector can be executed from the repository root with:

```sh
pytest idmefv2/connectors/clamav
```

If you prefer to run only the Clamav connector tests from the connector directory, use:

```sh
cd idmefv2/connectors/clamav
pytest
```
