...
Upon completion of a scan by `clamdscan`, the generated `metadata.json` will be analyzed by the connector and the virus detection data will be converted to a IDMEFv2 alert that will be uploaded to a IDMEFv2 server (either the testserver or the Concerto SIEM).
