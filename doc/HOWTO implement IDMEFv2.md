...
The first step when implementing IDMEFv2 is to inventory the data that your tool generates and that you want to include in the IDMEFv2 message. This inventory will be useful to define the templates of the IDMEFv2 messages generated by your tool.

You should:
- enumerate every data available in your tool
- for each data, define its type (string, integer, float, boolean, list, dict, enum, etc.)
- drop useless data that you don't want to include in the IDMEFv2 message. Keep in mind that the more you include in the IDMEFv2 message, the more useful will be the message.
- Map every field to a corresponding IDMEFv2 field, if possible. If no corresponding field exists, you can use the `"additionalProperties"` field of the IDMEFv2 message to include this data.
- When a IDMEFv2 field is an enum, you should map your tool's data to one of the enum values.

## Templates of the IDMEFv2 message

It is possible that your tool generates different types of alerts. In such case, you may identify various templates of IDMEFv2 messages, one for each type of alert. Each template will be a JSON object containing the IDMEFv2 fields that you want to include in the generated message.
